...
logger = logging.getLogger("libpebble2.communication.transports.lightblue")
...
class LightBlueTransport(BaseTransport):
    must_initialise = True

    def __init__(self, pebble_id, should_pair, debug_protocol=False, connection_process_timeout=60):
        self.connection_process_timeout = connection_process_timeout
        self.mac_address = pebble_id
        ...
        self.bt_socket_proc = Process(target=self._run)
    ...
    def send_packet(self, message, target=MessageTargetWatch()):
        ...
        try:
            ...
            logger.debug("LightBlue process has shut down (queue write)")
            raise
    ...
    def connect(self):
        logger.info("Starting lightblue proccess.")
        self.bt_socket_proc.start()
        self.bt_socket_proc.join()
        print self.bt_socket_proc.exitcode

        # wait for a successful connection from child process before returning to main process
        logger.info("Waiting...")
        self.bt_connected.wait(self.connection_process_timeout)
        if not self.bt_connected.is_set():
            raise LightBluePebbleError(self.mac_address,
                                       "Connection timed out, LightBlueProcess was provided {} seconds to complete "
                                       "connecting" % self.connection_process_timeout)
        self._connected = True

    def _run(self):
        logger.info("LightBlue process has started on pid %d" % os.getpid())
        # These are imported late to avoid the overhead when not using lightblue.
        ...
        def autodetect(self):
            ...
            if self.mac_address is not None and len(self.mac_address) is 4:
                ...
                logger.info("Going to get full address for device %s, ensure device is broadcasting." % self.mac_address)
                # scan for active devices
                devices = finddevices(timeout=8)

                for device in devices:
                    if re.search(r'Pebble ' + self.mac_address, device[1], re.IGNORECASE):
                        logger.debug("Found Pebble: %s @ %s" % (device[1], device[0]))
                ...
                try:
                    ...
                    logger.warn("failed to select a device in GUI selector")
        ...
        try:
            ...
            self._bts = lb_socket()
            print "connecting to {}".format(self.mac_address)
        ...
        logger.debug("Connection established to " + self.mac_address)
        ...
        while not self.bt_teardown.is_set():
            ...
            try:
                ...
                logger.debug("LightBlue Send: %r", send_data)
            ...
            if rec_data is not None and len(rec_data) == 4:
                ...
                try:
                    resp = rec_data + resp
                    self.rec_queue.put(resp)

                except (IOError, EOFError):
                    self.bt_teardown.set()
                    e = "Queue Error while recieving data"
                    pass
                logger.debug("LightBlue Read: %r " % resp)
    ...
    def read_packet(self):
        try:
            while True:
                try:
                    message = self.rec_queue.get()
                    return MessageTargetWatch(), message
                except Queue.Empty:
                    continue
        except:
            self.bt_teardown.set()
            if self.debug_protocol:
                logger.debug("LightBlue process has shutdown (queue read)")
            raise
